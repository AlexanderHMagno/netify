...
***********************************************************/
	if (!isset($_POST['x']) && !isset($_FILES['image']['name']) ){
		//Delete users temp image
			$temppath = '/assets/images/avatars/'.$profile_id.'_temp.jpeg';
...
***********************************************************/	
	...
	//Set Upload directory    
		$uploaddir = $_SERVER['DOCUMENT_ROOT'].'/socialNetwork/assets/images/avatars';
	//Set File name	
	...
	//Move the file to correct location
		...
		if (!$move) { 
			...
			$imgSrc= "http://localhost:8080/socialNetwork/assets/images/avatars/".$file_name; // the image to display in crop area
...
if (isset($_POST['x'])){
	...
	//the file type posted
		$type = $_POST['type'];	
	//the image src
		$src = 'assets/images/avatars/'.$_POST['src'];	
	...
	if($type == 'jpg' || $type == 'jpeg' || $type == 'JPG' || $type == 'JPEG'){	
		...
		//save the new cropped version
			imagejpeg($dst_r, "assets/images/avatars/".$finalname."n.jpeg", 90); 	
		...
		$result_path ="http://localhost:8080/socialNetwork/assets/images/avatars/".$finalname."n.jpeg";

		//Insert image into database
		$insert_pic_query = mysqli_query($con, "UPDATE users SET Profile_pic='$result_path' WHERE id='$user_id'");
		header("Location: "."user_section/settings.php");
...
<div class="main_column column">
    ...
    if($imgSrc){ //if an image has been uploaded display cropping area?>
	    ...
	    <div id="CroppingContainer" style="width:800px; max-height:600px; background-color:#FFF; margin:auto; position:relative; overflow:hidden; z-index:2001; padding-bottom:0px;">  
